...
/// highlighting.
///
pub fn render_diff_lines(diff_text: &str, styles: UiStyles<'_>, _width: u16) -> Vec<Line<'static>> {
    let rows = diff_line_views(diff_text);
    ...
    for row in rows {
        match row {
            DiffLineView::FileHeader { marker, path } => {
                result.push(Line::from(vec![
                    Span::raw(format!("  {marker} ")).fg(styles.dim()),
                    Span::raw(path).fg(styles.primary()).bold(),
                ]));
            }
            DiffLineView::Hunk {
                old_start,
            ...
            DiffLineView::Context {
                ...
                result.push(Line::from(vec![
                    ...
                    Span::raw(content).fg(styles.dim()),
                ]));
            }
            DiffLineView::Removed {
                ...
                if let Some(compare_to) = compare_to {
                    let (rm_spans, _) = word_diff_spans(
                        &content,
                        &compare_to,
                        removed_style,
                        added_style,
                    ...
                    line_spans.push(Span::styled("-", removed_style));
                    line_spans.extend(rm_spans);
                } else {
                    line_spans.push(Span::styled(format!("-{content}"), removed_style));
                }
            ...
            DiffLineView::Added {
                ...
                if let Some(compare_to) = compare_to {
                    let (_, add_spans) = word_diff_spans(
                        &compare_to,
                        &content,
                        removed_style,
                        added_style,
                    ...
                    line_spans.push(Span::styled("+", added_style));
                    line_spans.extend(add_spans);
                } else {
                    line_spans.push(Span::styled(format!("+{content}"), added_style));
                }
                result.push(Line::from(line_spans));
            }
        }
...
pub fn render_structured_diff(
    ...
    // ── Diff content ────────────────────────────────────────────
    let diff_lines = render_diff_lines(diff_text, styles, width);
