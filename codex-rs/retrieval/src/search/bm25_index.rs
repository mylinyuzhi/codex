...
impl SparseEmbedding {
    ...
    pub fn from_bm25_embedding(embedding: &bm25::Embedding<u32>) -> Self {
        ...
        for token_embedding in embedding.iter() {
            indices.push(token_embedding.index as i32);
...
impl Bm25Index {
    ...
    ///
    /// Reconstructs the index from persisted data.
    pub fn load(
        embeddings: HashMap<String, SparseEmbedding>,
        ...
        // Rebuild scorer from stored embeddings
        // Note: We can't fully reconstruct bm25::Embedding from SparseEmbedding
        // So we'll re-embed documents when they're queried
        // For now, store the sparse embeddings for persistence
