...
use super::shell_invoke::extract_bash_command;
use super::shell_invoke::try_parse_shell;
use super::shell_invoke::try_parse_word_only_commands_sequence;
...
pub fn parse_command(command: &[String]) -> Vec<CommandSummary> {
    ...
    if deduped
        .iter()
        .any(|cmd| matches!(cmd, CommandSummary::Unknown { .. }))
    {
...
fn single_unknown_for_command(command: &[String]) -> CommandSummary {
...
pub fn parse_command_impl(command: &[String]) -> Vec<CommandSummary> {
    ...
    for tokens in &parts {
        ...
        let parsed = match parsed {
            CommandSummary::Read { cmd, name, path } => {
...
fn simplify_once(commands: &[CommandSummary]) -> Option<Vec<CommandSummary>> {
    if commands.len() <= 1 {
        return None;
    }

    // echo ... && ...rest => ...rest
    if let CommandSummary::Unknown { cmd } = &commands[0]
    ...
    if let Some(idx) = commands
        .iter()
        .position(|pc| matches!(pc, CommandSummary::Unknown { cmd } if cmd == "true"))
    {
        let mut out = Vec::with_capacity(commands.len() - 1);
        out.extend_from_slice(&commands[..idx]);
        out.extend_from_slice(&commands[idx + 1..]);
...
fn parse_grep_like(main_cmd: &[String], args: &[String]) -> CommandSummary {
    let args_no_connector = trim_at_connector(args);
...
fn parse_shell_lc_commands(original: &[String]) -> Option<Vec<CommandSummary>> {
        ...
        && !all_commands.is_empty()
    {
        ...
        if filtered_commands.is_empty() {
            return Some(vec![CommandSummary::Unknown {
                cmd: script.to_string(),
            }]);
        }
        ...
        if commands.len() > 1 {
            commands.retain(|pc| !matches!(pc, CommandSummary::Unknown { cmd } if cmd == "true"));
        ...
        if commands.len() == 1 {
            ...
            commands = commands
                .into_iter()
                .map(|pc| match pc {
                    CommandSummary::Read { name, cmd, path } => {
                        if had_connectors {
                    ...
                    CommandSummary::ListFiles { path, cmd, .. } => {
                        if had_connectors {
                            CommandSummary::ListFiles { cmd, path }
                        } else {
                            CommandSummary::ListFiles {
                                cmd: shlex_join(&script_tokens),
                                path,
                            }
                        }
                    ...
                    CommandSummary::Search {
                        ...
                        if had_connectors {
                            CommandSummary::Search { cmd, query, path }
                        } else {
                            CommandSummary::Search {
                                cmd: shlex_join(&script_tokens),
                                query,
    ...
    Some(vec![CommandSummary::Unknown {
        cmd: script.to_string(),
    }])
}
...
fn summarize_main_tokens(main_cmd: &[String]) -> CommandSummary {
