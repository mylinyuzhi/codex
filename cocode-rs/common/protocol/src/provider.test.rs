...
#[test]
fn test_provider_api_default() {
    assert_eq!(ProviderApi::default(), ProviderApi::Openai);
}

#[test]
fn test_provider_api_display() {
    assert_eq!(ProviderApi::Openai.to_string(), "openai");
    assert_eq!(ProviderApi::Anthropic.to_string(), "anthropic");
    assert_eq!(ProviderApi::Gemini.to_string(), "gemini");
    assert_eq!(ProviderApi::Volcengine.to_string(), "volcengine");
    assert_eq!(ProviderApi::Zai.to_string(), "zai");
    assert_eq!(ProviderApi::OpenaiCompat.to_string(), "openai_compat");
}
...
fn test_provider_info_new() {
    let info = ProviderInfo::new("OpenAI", ProviderApi::Openai, "https://api.openai.com/v1");
    assert_eq!(info.name, "OpenAI");
    assert_eq!(info.api, ProviderApi::Openai);
...
fn test_provider_info_builders() {
    ...
    let info = ProviderInfo::new("Test", ProviderApi::Openai, "https://api.test.com")
...
fn test_provider_info_serde() {
    let info = ProviderInfo::new("Test", ProviderApi::Anthropic, "https://api.anthropic.com")
    ...
    assert!(json.contains("\"api\":\"anthropic\""));
...
fn test_provider_info_equality() {
    let info1 =
        ProviderInfo::new("Test", ProviderApi::Openai, "https://api.test.com").with_api_key("key1");
    let info2 =
        ProviderInfo::new("Test", ProviderApi::Openai, "https://api.test.com").with_api_key("key1");
    let info3 =
        ProviderInfo::new("Test", ProviderApi::Openai, "https://api.test.com").with_api_key("key2");
