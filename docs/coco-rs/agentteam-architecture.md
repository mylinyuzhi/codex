...
| Item | Status | Notes |
|---|---|---|
| `coordinator_user_context` (workerToolsContext) unwired | open | Pure helper exists in `core/subagent/src/coordinator_mode.rs:170-208` but no consumer in `app/query`, `core/system-reminder`, or `core/context`. Closed by unit 1 of this batch |
| Layer 4 `parent_tool_filter` intersection | open | `core/subagent/src/filter.rs` does not intersect the parent's tool_filter with the subagent's allowed_tools. Closed by unit 2 of this batch |
| SendMessage cross-process routing (`bridge:` + `uds:`) | open | Two prefixes from TS `SendMessageTool.ts:643-755` not handled; `ReplBridge` in `coco-bridge` already exists. Closed by unit 3 of this batch |
| Post-turn `<task-notification>` / `<session-mode-switch>` consumers | open | Pure helpers exist but no caller in `engine_finalize_turn`. Closed by unit 4 of this batch |
| MCP-sourced skills builder + lifecycle | open | `SkillSource::Mcp` variant exists in types but no registry/consumer. Closed by unit 5 of this batch |
| Plan Mode V2 multi-agent concurrency gate | open | Currently hardcoded `MAX_IN_PROCESS_AGENTS=16`; no plan-mode awareness. Closed by unit 6 of this batch (config-only; subscription-tier dispatch is scope-cut) |
| Skill `paths:` glob conditional activation | open (P2) | Closed by unit 7 of this batch |
| Skill watcher stability + ConfigChange hook + `additional_dirs` | open (P2) | Closed by unit 8 of this batch |
| `Prompt::File` parts rendering in command runners | open (P2) | Closed by unit 9 of this batch |
| `coco plugin install` marketplace + git-URL paths | open (P2) | Closed by unit 10 of this batch |
...
> **Deferred (not in this batch):** event-time reminder producers — six
> emitters were noted as deferred in `current-gap-fix-plan.md#p27`.
> Inventory is NOT completed in this batch; tracked as a follow-up.

### Verified closed

Rows previously listed in the open section that the deep audit confirmed
have landed. Evidence column points at the file:line that the audit
cross-checked.

| Item | Evidence |
|---|---|
| Semantic compaction in runner_loop | `coco-rs/coordinator/src/agent_handle/teammate_engine.rs:95-187` runs full LLM `compact_conversation` via summarize callback through wrapped engine. NOT just a sliding-window safety valve — that fires only when the engine declines compaction |
| Memory snapshot file IO bodies | `coco-rs/memory/src/agent_memory_snapshot.rs` exists with full IO; wired from `coco-rs/app/cli/src/{session_runtime.rs:1188, agent_handle_factory.rs:286-319, bin_handlers/agents.rs:25, sdk_server/cli_bootstrap.rs:156}` |
| Handoff classifier orchestration call site | `coco-rs/coordinator/src/agent_handle/handoff.rs:18-73` fires both stage-1 and stage-2 LLM calls via `side_query`; `spawn.rs:972` invokes after every spawn |
| Plan-approval block-and-await caller | `coco-rs/coordinator/src/runner_loop.rs:293, 411-450` blocks between plan-write turn and first impl turn. Fully wired |
| Permission-bridge poll loop on workers | Workers inherit the leader's `ToolPermissionBridge` via `wire_engine` (SDK / TUI); `MailboxPermissionBridge` covers cross-process pane. The orphaned in-process mpsc circuit was deleted in the post-D cleanup pass after deep review surfaced it as dead code |
| Leader-side TUI permission overlay (P0) | `TuiPermissionBridge` installed on `SessionRuntime` for TUI mode; emits `TuiOnlyEvent::ApprovalRequired` and resolves via `UserCommand::ApprovalResponse` |
| `forkedAgent.ts` post-turn cache slot (D8 + D1/D2) | `CacheSafeParams` slot + `ForkDispatcher` trait + production CLI impl; `/btw` and `promptSuggestion` consume it |
| TUI components for coordinator/teammate (E4) | `CoordinatorPanel`, `TeammateViewHeader`, `SubagentPanel`, `TeammateSpinner` all wired in `render.rs` |
| AgentSummary — one-shot at completion (E3) | Via `SideQueryHandle`; populates `SubAgentState.last_message` |
| AgentSummary — periodic (every 30 s) (bg path) | The live `TaskOutputDelta` streaming infrastructure unblocked this — the bg AgentTool path now spawns a 30s timer per spawn that reads `AgentTaskRegistry::read_output(task_id)`, hands the recent tail to `side_query` for summarization, and writes the result onto `SubAgentState.last_message` so the panel updates while the agent is running. Cancellation observes the same token as the engine driver. Sync (foreground) AgentTool spawns block the parent loop and don't need periodic summarization — the one-shot at-completion summary (E3) covers them |
| `<task-notification>` ingestion call site | `runner_loop` emits via `render_task_notification` on worker terminate when coordinator mode is active |
| AgentTool reads `AgentDefinition.background` (D5 + P1') | OR'd into `run_in_background` along with `is_coordinator_mode` and gated by `COCO_BACKGROUND_TASKS_DISABLE`; in-process-teammate guard throws on conflict |
| AgentTool background task registration | `coco_cli::task_runtime::TaskRuntime` implements both `coco_tool_runtime::TaskHandle` (read/control) and `AgentTaskRegistry` (registration). The same `Arc` flows into the engine via `wire_engine` and into `SwarmAgentHandle::set_task_registry`, so AgentTool's bg path registers spawns through the same `TaskManager` that `TaskGet` / `TaskOutput` / `TaskStop` read from. The bg dispatch returns the `task_id` as the response's `agent_id` so the model can address the spawn directly. `kill_task` flips a per-task `CancellationToken` that the spawn's `tokio::select!` observes |
| AgentTool background — live `TaskOutputDelta` streaming | New `AgentQueryConfig.event_tx` field carries an optional `mpsc::Sender<CoreEvent>` from caller through to the adapter. The bg AgentTool path allocates a per-task channel, drains `Stream::TextDelta` events into the task's output buffer via `AgentTaskRegistry::append_output`, so `TaskOutput` returns mid-flight text immediately rather than waiting for completion. `mark_completed` no longer double-appends `response_text` since the deltas already streamed |
| AgentTool background — `--resume` persistence + model trigger (TS-aligned, model-driven) | TS-faithful resume via per-agent JSONL transcript + sidecar metadata. New `AgentTranscriptStore` trait in `coco-tool-runtime` (registration side decoupled from `coco-session` to respect L5-app vs L5-root layering); `SessionAgentTranscriptStore` impl in `app/cli` wraps `coco_session::TranscriptStore`. Storage layout matches TS `<sessions_dir>/<session_id>/subagents/agent-<id>.{jsonl,meta.json}`. Bg path writes meta on register + transcript on completion. **Model-driven trigger** (TS `SendMessageTool.ts:822-872` parity): `SendMessageTool::execute` checks `ctx.task_handle.get_task_status(to)` first; when the target's status is terminal (`Completed` / `Failed` / `Killed`), it dispatches `ctx.agent.resume_agent(to, message, session_id)` instead of routing through the team mailbox. The model just keeps using `SendMessage` and resume is automatic on stopped agents. `resume_agent` reads transcript + meta, filters via `coco_subagent::filter_transcript`, and dispatches a new spawn with `SpawnMode::Fork` carrying the prior history as `parent_messages`. Earlier "engine state isn't recoverable" framing was wrong — TS doesn't recover the streaming connection either, only the conversation history, which IS recoverable |
| `/clear` clears post-turn slots (D4) | `ToolAppState::default()` reset clears `prompt_suggestion`; cache slot is engine-local and rebuilds per turn |
| `runner_loop.rs` 800-LoC cap (P1) | Split into `runner_loop_mailbox_permission` + `runner_loop_wait` + `runner_loop_notify`; main file at 761 LoC |
| `core/tools/src/tools/agent.rs` 800-LoC cap | Old 1011-LoC monolith split into per-tool modules under `tools/agent/` (`agent_tool.rs`, `skill_tool.rs`, `send_message_tool.rs`, `team_tools.rs`); largest module 591 LoC |
| `coordinator/src/agent_handle_spawn.rs` 800-LoC cap | Old 854-LoC file split into `agent_handle/{mod,spawn,handoff,resume}.rs`; largest module 585 LoC |
| Per-`AgentTypeId` color cache | `pane::layout::assign_agent_type_color` mirrors TS `agentColorManager.ts`; populated by `spawn_subagent` so `SubagentPanel` renders all `Explore` spawns in the same color |
| Per-agent MCP server validation (fail-fast) | `AgentTool::execute` rejects spawns whose declared `mcp_servers` aren't connected via `McpHandle::connected_servers`. Fail-fast (not a 30 s poll) since coco-rs settles MCP boot at session start; the model retries with corrected `mcp_servers` |
| `#[non_exhaustive]` on cross-crate enums | Added to `HandoffClassification`, `SessionMode`, `SessionModeSwitch`, `TaskNotificationStatus` — were public crate-root re-exports without the attribute |
| Resume mistakenly used `SpawnMode::Fork` (rewrites tool_result to `FORK_PLACEHOLDER`) | New `SpawnMode::Resume { parent_messages }` variant. `spawn.rs` keeps `tool_result` blocks verbatim under Resume so the resumed child can continue the conversation; system_prompt is left empty so the engine rebuilds from `definition`. Mirrors TS `resumeAgent.ts:resumeAgentBackground` for non-fork agent types |
| Dangling `SubAgentState` entry on validation failure | `spawn_subagent` now commits the agent-state push only after worktree + execution-engine gates pass; earlier code left a Pending entry visible to `query_agent_status` / `SubagentPanel` whenever validation failed |
| `SendMessageTool` auto-resume with empty `session_id` | Reject upfront with a clear "parent session id is unavailable" error instead of forwarding to `resume_agent` and surfacing a confusing inner "no metadata" failure |
| `render_block_message` dangling em-dash on empty reason | Bare `"BLOCKED"` (no `:`) used to render `"SECURITY: subagent output withheld — "`; now collapses to `"unspecified safety concern"` so the payload is a self-contained sentence |
| Coordinator never sees worker's first-turn errors | `runner_loop` query-error path was an early `return` that ran `on_teammate_stop` but skipped the coordinator-mode `<task-notification>` send. Refactored to stash the error in `run_error: Option<String>` and `break` to the unified cleanup, which now emits `TaskNotificationStatus::Failed` (with the error in `result`) when `is_coordinator_mode`, and `Completed` otherwise |
| Mailbox lock retries / backoff envelope drifted from TS | Aligned `mailbox/lock.rs` to TS `proper-lockfile` config (`retries: 10, minTimeout: 5, maxTimeout: 100`): 10 retries, exponential 5 → 100 ms cap with `[0.5×, 1.5×)` jitter (TS achieves equivalent via `randomize: true`). Earlier 30 retries × 50 ms cap × jitter could block ~1.5 s under contention vs TS's ~1 s envelope |
| Pink color used wrong tmux palette index | `agent_color_to_tmux(Pink)` returned `colour213`; TS `TmuxBackend.ts:67` uses `colour205`. Fixed plus updated the parity test |
| Pane-active-border-style not set on tmux 3.2+ | `TmuxBackend::set_pane_border_color` only ran one `select-pane -P "pane-border-style=..."`. TS does a 3-step sequence: `select-pane -P bg=default,fg=...`, `set-option -p ... pane-border-style ...`, `set-option -p ... pane-active-border-style ...` so the colour stays applied whether the pane is active or inactive. Rust now mirrors the full sequence |
| Dead `pending_user_messages: Vec<String>` scaffold | Local Vec was passed into `wait_for_next_prompt_or_shutdown` but never written to anywhere — TS reads from `task.pendingUserMessages` populated by transcript-view UI, which coco-rs doesn't yet have. Removed the parameter + branch; `wait_for_next_prompt_or_shutdown` doc-comment now points to the future port (per-`agent_id` `mpsc::UnboundedReceiver` registry on `InProcessAgentRunner`) so when the TUI lands the seam is obvious |
