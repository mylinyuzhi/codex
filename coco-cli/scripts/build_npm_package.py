...
PACKAGE_EXPANSIONS: dict[str, list[str]] = {
    "coco-cli": ["coco-cli", *COCO_PLATFORM_PACKAGES],
}

PACKAGE_NATIVE_COMPONENTS: dict[str, list[str]] = {
    "coco-cli": [],
    "coco-cli-linux-x64": ["coco"],
    "coco-cli-linux-arm64": ["coco"],
    "coco-cli-darwin-arm64": ["coco"],
}

PACKAGE_TARGET_FILTERS: dict[str, str] = {
    package_name: package_config["target_triple"]
    for package_name, package_config in COCO_PLATFORM_PACKAGES.items()
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="coco-cli",
        help="Which npm package to stage (default: coco-cli).",
    )
    parser.add_argument(
        "--version",
        ...
        if release_version:
            ...
            if package == "coco-cli":
                print(
                    ...
                    "Verify the CLI:\n"
                    f"    node {staging_dir_str}/bin/coco-cli.js --version\n"
                    f"    node {staging_dir_str}/bin/coco-cli.js --help\n\n"
                )
            elif package in COCO_PLATFORM_PACKAGES:
                print(
                    f"Staged version {version} for release in {staging_dir_str}\n\n"
...
def prepare_staging_dir(staging_dir: Path | None) -> tuple[Path, bool]:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="coco-npm-stage-"))
...
def stage_sources(staging_dir: Path, version: str, package: str) -> None:
    ...
    if package == "coco-cli":
        ...
        shutil.copy2(COCO_CLI_ROOT / "bin" / "coco-cli.js", bin_dir / "coco-cli.js")

        readme_src = COCO_CLI_ROOT / "README.md"
        if readme_src.exists():
            shutil.copy2(readme_src, staging_dir / "README.md")

        package_json_path = COCO_CLI_ROOT / "package.json"
    elif package in COCO_PLATFORM_PACKAGES:
        platform_package = COCO_PLATFORM_PACKAGES[package]
        ...
        with open(COCO_CLI_ROOT / "package.json", "r", encoding="utf-8") as fh:
        ...
        package_json = {
            ...
            "files": ["vendor"],
            "repository": coco_package_json.get("repository"),
        }

        engines = coco_package_json.get("engines")
        if isinstance(engines, dict):
            package_json["engines"] = engines

        package_manager = coco_package_json.get("packageManager")
...
def run_npm_pack(staging_dir: Path, output_path: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="coco-npm-pack-") as pack_dir_str:
