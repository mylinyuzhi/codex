...
pub fn prompt_to_contents(prompt: &Prompt) -> Vec<Content> {
    ...
    // Build call_id -> function_name mapping for FunctionCallOutput handling
    let call_id_to_name: HashMap<String, String> = prompt
        .input
        .iter()
        .filter_map(|item| {
    ...
    for item in &prompt.input {
        match item {
            ...
            ResponseItem::Reasoning {
                ...
                if let Some(content) = extract_full_response_content(enc) {
            ...
            ResponseItem::FunctionCallOutput { call_id, output } => {
                ...
                // Look up function name from corresponding FunctionCall
                let function_name = call_id_to_name.get(call_id).cloned();
                let response_id = if is_client_generated_call_id(call_id) {
                    None // Strip client-generated ID, rely on name for matching
                } else {
                    Some(call_id.clone())
                };
...
mod tests {
    ...
    fn test_prompt_to_contents_function_output() {
        let prompt = Prompt {
            instructions: String::new(),
            input: vec![
                // FunctionCall comes first, provides name mapping
                ResponseItem::FunctionCall {
                    id: Some("resp-1".to_string()),
                    name: "get_weather".to_string(),
                    arguments: "{}".to_string(),
                    call_id: "server_call_123".to_string(),
                },
    ...
    fn test_prompt_to_contents_client_generated_id_stripped() {
        let client_call_id = format!("{}test_uuid", CLIENT_GENERATED_CALL_ID_PREFIX);

        let prompt = Prompt {
            instructions: String::new(),
            input: vec![
                // FunctionCall with client-generated ID, provides name mapping
                ResponseItem::FunctionCall {
                    id: Some("resp-1".to_string()),
        ...
        assert_eq!(
            ...
            "Name should be set for matching when ID is stripped"
        );
    }
    ...
    fn test_response_to_events_stores_full_response() {
        ...
        let (events, resp_id) = response_to_events(&response);
    ...
    fn test_roundtrip_with_full_response() {
        ...
        // Convert to events
        let (events, _) = response_to_events(&response);
    ...
    fn test_server_call_id_not_detected_as_client() {
        ...
        assert!(!is_client_generated_call_id("call_abc"));
    }
