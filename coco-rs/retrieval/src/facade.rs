...
//!
//! ```ignore
//! use coco_retrieval::{RetrievalFacade, FacadeBuilder, RetrievalFeatures};
//!
//! // Standard: Get facade for current working directory (cached)
//! let facade = RetrievalFacade::for_workdir(&cwd, &coco_home).await?;
//!
...
impl RetrievalFacade {
    ...
    pub async fn for_workdir(workdir: &Path, coco_home: &Path) -> Result<Arc<Self>> {
    ...
    pub fn is_configured(workdir: &Path, coco_home: &Path) -> bool {
