...
use coco_retrieval::EventConsumer;
use coco_retrieval::FacadeBuilder;
use coco_retrieval::JsonLinesConsumer;
use coco_retrieval::LoggingConsumer;
use coco_retrieval::RebuildMode;
use coco_retrieval::RepoMapRequest;
use coco_retrieval::RetrievalConfig;
use coco_retrieval::RetrievalFacade;
use coco_retrieval::SnippetStorage;
use coco_retrieval::SqliteStore;
use coco_retrieval::SymbolQuery;
use coco_retrieval::event_emitter;
use coco_retrieval::events::LogLevel;
use coco_retrieval::indexing::IndexStatus;
...
fn bm25_features() -> coco_retrieval::RetrievalFeatures {
    coco_retrieval::RetrievalFeatures::STANDARD
}

/// Create features for hybrid search (BM25 + vector if available).
fn hybrid_features() -> coco_retrieval::RetrievalFeatures {
    coco_retrieval::RetrievalFeatures::FULL
}
...
async fn main() -> anyhow::Result<()> {
    ...
    // Resolve coco home directory
    let coco_home = coco_retrieval::find_coco_home();
    ...
    let config = if let Some(config_path) = &cli.config {
        ...
        RetrievalConfig::load(&workdir, &coco_home)?
    };
    ...
    let _log_guard = init_tracing(&config.logging, cli.verbose, &coco_home);
    ...
    if cli_mode {
        ...
        match cli.command {
            Some(Command::Repl) => run_repl(&workdir, &config, cli.config.as_deref()).await?,
        ...
        let mut app = coco_retrieval::tui::App::new(tui_config, service);
        if let Some(reason) = not_enabled_reason {
            app.error_banner = Some(reason);
        }

        // Initialize terminal and run
        let mut terminal = coco_retrieval::tui::init_terminal()?;
        let result = app.run(&mut terminal).await;
        coco_retrieval::tui::restore_terminal()?;
        result?;
    }
...
///
/// Logs are written to `{coco_home}/log/retrieval.log`.
...
fn init_tracing(
    config_logging: &coco_utils_common::LoggingConfig,
    verbose: u8,
    coco_home: &Path,
) -> Option<WorkerGuard> {
    ...
    let log_dir = coco_home.join("log");
    ...
    let logging_config = if verbose > 0 {
        ...
        coco_utils_common::LoggingConfig::with_level(level)
    } else {
        config_logging.clone()
    };

    // Use coco-utils logging infrastructure for timezone-aware timestamps
    let file_layer = coco_utils_common::configure_fmt_layer!(
...
fn init_tracing_stderr(
    ...
    // Use coco-utils logging infrastructure for consistent formatting
    let stderr_layer = coco_utils_common::configure_fmt_layer!(
        tracing_subscriber::fmt::layer()
...
async fn run_command(cmd: Command, workdir: &Path, config: &RetrievalConfig) -> anyhow::Result<()> {
    // Create service with workdir set for operations commands
    let mut service_config = config.clone();
    service_config.workdir = Some(workdir.to_path_buf());
