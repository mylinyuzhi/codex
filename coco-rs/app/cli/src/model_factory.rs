//! Multi-provider `RuntimeConfig + ModelSpec → Arc<dyn LanguageModelV4>` factory.
//!
...
/// Build an `Arc<dyn LanguageModelV4>` for the (provider, model) pair
...
fn build_openai(
    ...
    // `ProviderV4::language_model` defaults to Responses, but users
    // who configure `wire_api: chat` (e.g. for Azure Chat Completions
    // routing) expect Chat. Dispatch explicitly.
    let model: Arc<dyn LanguageModelV4> = match provider_cfg.wire_api {
