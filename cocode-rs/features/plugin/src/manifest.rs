...
/// The hidden plugin directory name (cocode branding).
pub const PLUGIN_DIR: &str = ".cocode-plugin";
...
impl PluginManifest {
    ...
    ///
    /// Checks for `.cocode-plugin/plugin.json` first, then falls back to
    /// `plugin.json` at the root.
    pub fn from_dir(dir: &Path) -> Result<Self> {
        // Check .cocode-plugin/plugin.json first
        let hidden_path = dir.join(PLUGIN_DIR).join(PLUGIN_JSON);
        if hidden_path.exists() {
            return Self::from_file(&hidden_path);
        }

        // Fall back to plugin.json at root
