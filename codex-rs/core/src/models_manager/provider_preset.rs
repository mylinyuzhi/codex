...
///
/// Note: `derive_model_family()` should be called on the provider before calling
/// this function to ensure `model_family` is populated for `default_reasoning_effort`.
pub fn provider_to_preset(provider_id: &str, provider: &ModelProviderInfo) -> ModelPreset {
    ...
    // Get default_reasoning_effort from model_family if available, else None
...
mod tests {
    ...
    fn test_provider_to_preset_default_reasoning_effort_none() {
        // Without model_family, default_reasoning_effort should be None
        let provider = ModelProviderInfo {
            name: "Custom".to_string(),
            base_url: None,
            ext: ModelProviderInfoExt::default(),
