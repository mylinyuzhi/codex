...
impl LanceDbStore {
    ...
    fn get_schema(&self) -> Schema {
        Schema::new(vec![
            ...
            Field::new("branch", DataType::Utf8, true), // nullable
    ...
    fn chunks_to_batch(&self, chunks: &[CodeChunk]) -> Result<RecordBatch> {
        ...
        let workspaces: Vec<&str> = chunks
            ...
            .collect();
        let branches: Vec<Option<&str>> = chunks.iter().map(|c| c.branch.as_deref()).collect();
    ...
    fn batch_to_chunks(batch: &RecordBatch) -> Result<Vec<CodeChunk>> {
        ...
        for i in 0..batch.num_rows() {
            ...
            let branch = branches.and_then(|b| {
                if b.is_null(i) {
                    None
                } else {
    ...
    pub async fn get_workspace_files(
        ...
        let filter = if let Some(b) = branch {
            let escaped_branch = b.replace('\'', "''");
            format!(
                "workspace = '{}' AND branch = '{}'",
                escaped_workspace, escaped_branch
            )
        } else {
            format!("workspace = '{}'", escaped_workspace)
        };
...
mod tests {
    ...
    async fn test_get_workspace_files() {
        ...
        let files = store.get_workspace_files("ws", None).await.unwrap();
    ...
    async fn test_delete_workspace() {
        ...
        let files = store.get_workspace_files("ws2", None).await.unwrap();
