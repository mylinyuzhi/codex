...
//! ## Full JSON (advanced)
//! ```text
//! COCODE_API_TEST_{PROVIDER}_CONFIG={"name":"openai","type":"openai",...}
//! ```
//!
//! # Capability Gating
//!
...
fn load_capabilities(provider: &str) -> HashSet<String> {
    ...
    let config_name = config_provider_name(provider);
    if config_name != provider {
        if let Some(caps) = get_env(config_name, "CAPABILITIES") {
            return parse_capabilities(&caps);
        }
    }
...
fn provider_type_for(provider: &str) -> ProviderType {
    match provider {
        "openai" | "openai_chat" => ProviderType::Openai,
...
fn load_from_flat(provider: &str, capabilities: HashSet<String>) -> Option<ProviderTestConfig> {
    ...
    let provider_type = provider_type_for(provider);
    let model_slug = model.unwrap_or_default();

    let base_url = base_url.unwrap_or_else(|| default_base_url(provider_type).to_string());
    ...
    let mut provider_info = ProviderInfo::new(provider, provider_type, base_url)
    ...
    if user_agent.is_some() || options.contains_key("headers") {
        ...
        if let Some(ua) = &user_agent {
            if let Some(map) = headers_obj.as_object_mut() {
