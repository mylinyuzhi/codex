...
AI-friendly LSP client library for coco-rs with symbol name resolution.

## Overview

The `coco-lsp` crate provides a high-level abstraction over raw LSP (Language Server Protocol), enabling **symbol name-based queries** instead of exact line/column positions. This makes it ideal for AI-powered code analysis and navigation.
...
```rust
use coco_lsp::{LspServersConfig, LspServerManager, DiagnosticsStore, SymbolKind};
use std::sync::Arc;
use std::path::Path;
