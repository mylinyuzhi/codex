...
fn test_load_multiple_provider_files() {
    ...
    // First file
    let file1 = r#"[
        {"name": "openai", "api": "openai", "base_url": "https://api.openai.com/v1", "models": []}
    ]"#;
    std::fs::write(temp_dir.path().join("openai_provider.json"), file1).unwrap();

    // Second file
    let file2 = r#"[
        {"name": "anthropic", "api": "anthropic", "base_url": "https://api.anthropic.com", "models": []}
    ]"#;
...
fn test_load_provider_files_duplicate_error() {
    ...
    // First file
    let file1 = r#"[{"name": "openai", "api": "openai", "base_url": "https://api.openai.com/v1", "models": []}]"#;
    std::fs::write(temp_dir.path().join("a_provider.json"), file1).unwrap();

    // Second file with same name
    let file2 = r#"[{"name": "openai", "api": "openai", "base_url": "https://other.com/v1", "models": []}]"#;
...
fn test_load_all() {
    ...
    let providers =
        r#"[{"name": "test", "api": "openai", "base_url": "https://test.com", "models": []}]"#;
