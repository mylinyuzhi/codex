...
pub struct SessionState {
    ...
    /// Auto memory extraction coordinator (persists across turns).
    ///
    /// The coordinator manages in_progress state and coalescing for the
    /// fire-and-forget extraction subagent. Must persist across turns
    /// because the AgentLoop builder is recreated each turn.
    auto_memory_extraction:
        Option<Arc<cocode_loop::auto_memory_extraction::AutoMemoryExtractionCoordinator>>,
    ...
    /// Task ledger for team task claiming and dependencies.
    #[allow(dead_code)] // Wired into agent loop when team polling is connected
    task_ledger: Arc<cocode_team::TaskLedger>,

    /// Fast path for in-process message delivery.
    #[allow(dead_code)] // Used by SendMessage tool when fast path is connected
    fast_path: Arc<cocode_team::FastPath>,
...
impl SessionState {
    ...
    pub async fn new(session: Session, config: Arc<Config>) -> anyhow::Result<Self> {
        ...
        let (team_store, team_mailbox, task_ledger, fast_path) =
            cocode_tools::builtin::create_default_team_stores();
        if let Err(e) = team_store.load_from_disk().await {
            tracing::warn!(error = %e, "Failed to load persisted teams from disk");
        }
        if let Err(e) = task_ledger.load_from_disk().await {
            tracing::warn!(error = %e, "Failed to load persisted task ledgers from disk");
        }
        ...
        let resolved_auto_memory = cocode_auto_memory::resolve_auto_memory_config(
            ...
            config.features.enabled(Feature::TeamMemory),
        );
        let auto_memory_state = cocode_auto_memory::AutoMemoryState::new_arc(resolved_auto_memory);

        // This persists across turns so the coordinator can track in_progress state.
        let auto_memory_extraction = if auto_memory_state.config.memory_extraction_enabled {
            Some(
                cocode_loop::auto_memory_extraction::AutoMemoryExtractionCoordinator::new_arc(
                    auto_memory_state.config.directory.clone(),
                ),
            )
        } else {
            None
    ...
    /// Clean up orphaned worktrees matching the `agent/task-*` naming convention.
    ///
    /// Delegates to `cocode_git::cleanup_orphaned_worktrees` which removes the
    /// worktrees, deletes their branches, and prunes stale bookkeeping entries.
    async fn cleanup_orphaned_worktrees(cwd: &std::path::Path) {
        let cwd = cwd.to_path_buf();
        let count =
            tokio::task::spawn_blocking(move || cocode_git::cleanup_orphaned_worktrees(&cwd))
                .await
                .unwrap_or(0);
        if count > 0 {
            tracing::info!(count, "Cleaned up orphaned agent worktrees");
        }
    }
...
// Domain-specific operations (impl blocks in separate files)
#[path = "context_ops.rs"]
mod context_ops;
#[path = "permission_ops.rs"]
mod permission_ops;
#[path = "turn_runner.rs"]
mod turn_runner;
