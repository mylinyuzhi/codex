...
impl ProviderAdapter for GptAdapter {
    ...
    fn transform_request(
        ...
        // Add output_schema if present (for structured outputs)
        // Use create_text_param_for_request with verbosity=None since GptAdapter
        // doesn't handle verbosity (it's handled by the gateway/provider)
        if let Some(text_controls) = create_text_param_for_request(None, &prompt.output_schema) {
...
mod tests {
    ...
    fn test_transform_request_with_output_schema() {
        ...
        // Verify verbosity is not present (since we passed None)
        assert!(text.get("verbosity").is_none(), "verbosity should not be present");
    }
