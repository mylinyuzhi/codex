...
fn toggle_opens_transcript_when_no_overlay_active() {
    ...
    toggle(&mut state);
    assert!(matches!(state.ui.overlay, Some(Overlay::Transcript(_))));
}
...
fn transcript_overlay_defaults_show_all_to_true() {
    let mut state = AppState::new();
    toggle(&mut state);
    let overlay = state.ui.overlay.as_ref().expect("transcript opened");
...
fn toggle_show_all_flips_when_transcript_active() {
    ...
    let Some(Overlay::Transcript(ref t)) = state.ui.overlay else {
        panic!("expected Transcript overlay");
    };
