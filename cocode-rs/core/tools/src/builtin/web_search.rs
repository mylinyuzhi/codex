...
/// DuckDuckGo HTML result link pattern
#[allow(clippy::expect_used)]
static LINK_RE: LazyLock<regex_lite::Regex> = LazyLock::new(|| {
    regex_lite::Regex::new(r#"class="result__a"[^>]*href="([^"]+)"[^>]*>([^<]+)</a>"#)
        .expect("link regex pattern is valid")
});

/// DuckDuckGo HTML snippet pattern
#[allow(clippy::expect_used)]
static SNIPPET_RE: LazyLock<regex_lite::Regex> = LazyLock::new(|| {
    regex_lite::Regex::new(r#"class="result__snippet"[^>]*>([^<]+)"#)
