//! LSP client library for coco-rs
//!
...
/// `coco_home` as a parameter instead — callers are responsible for resolving it.
pub use coco_utils_common::find_coco_home;
...
/// # Example
/// ```ignore
/// use coco_lsp::create_manager;
///
/// // Create manager for current directory
/// let manager = create_manager(None, Some(std::env::current_dir().unwrap()));
/// ```
pub fn create_manager(
    coco_home: Option<&std::path::Path>,
    cwd: Option<PathBuf>,
) -> Arc<LspServerManager> {
    let lsp_config = LspServersConfig::load(coco_home, cwd.as_deref());
    let diagnostics = Arc::new(DiagnosticsStore::new());
    Arc::new(LspServerManager::new(
        lsp_config,
        coco_home.map(std::path::Path::to_path_buf),
        cwd,
        diagnostics,
    ))
}
