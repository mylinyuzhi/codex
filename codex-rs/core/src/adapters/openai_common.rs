...
//!
//! Parser: [`parse_chat_completions_chunk()`]
//! Source: Extracted from `chat_completions.rs:process_chat_sse()`
//!
//! **Format**:
//! ```json
//! {
...
//!
//! **Special signals**:
//! - `[DONE]` - String literal indicating stream end
//!
...
impl ChatCompletionsParserState {
    ...
    ///   "choices": [{
    ///     "delta": {
    ///       "content": "text",      // Optional: assistant text
    ///       "reasoning": "...",     // Optional: reasoning content
    ///       "tool_calls": [...]     // Optional: tool call fragments
    ///     },
    ///     "finish_reason": "stop"   // Optional: completion signal
    ///   }]
    /// }
    /// ```
    ///
    /// Or the special completion signal: `[DONE]`
...
///
/// ```rust,ignore
/// let mut state = ResponsesApiParserState::new();
/// let events = state.parse_chunk(r#"{"event":"response.output_text.delta","delta":"Hello"}"#)?;
/// ```
#[derive(Debug, Clone, Serialize, Deserialize)]
