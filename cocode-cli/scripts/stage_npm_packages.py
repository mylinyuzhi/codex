#!/usr/bin/env python3
"""Stage one or more Cocode npm packages for release."""
...
# TODO: Set to actual workflow name once CI is configured.
WORKFLOW_NAME = ""
# TODO: Set to actual GitHub repo once configured (e.g. "org/cocode").
GITHUB_REPO = ""

_SPEC = importlib.util.spec_from_file_location("cocode_build_npm_package", BUILD_SCRIPT)
...
COCODE_PLATFORM_PACKAGES = getattr(_BUILD_MODULE, "COCODE_PLATFORM_PACKAGES", {})
...
def resolve_release_workflow(version: str) -> dict:
    if not GITHUB_REPO or not WORKFLOW_NAME:
        raise RuntimeError(
            "GITHUB_REPO and WORKFLOW_NAME must be configured at the top of stage_npm_packages.py."
        )
...
def tarball_name_for_package(package: str, version: str) -> str:
    if package in COCODE_PLATFORM_PACKAGES:
        platform = package.removeprefix("cocode-cli-")
        return f"cocode-cli-npm-{platform}-{version}.tgz"
...
def main() -> int:
    ...
    try:
        if native_components:
            ...
            vendor_temp_root = Path(tempfile.mkdtemp(prefix="cocode-npm-native-", dir=runner_temp))
