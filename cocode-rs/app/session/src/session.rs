...
impl Session {
    ...
    pub fn primary_model(&self) -> Option<&RoleSelection> {
        self.selections.get(ModelRole::Main)
    }
    ...
    /// Get the primary provider api.
    ///
    /// Returns the provider api from the main role selection.
    pub fn provider_api(&self) -> Option<ProviderApi> {
        self.selections.get(ModelRole::Main).map(|s| s.model.api)
    }
...
impl SessionBuilder {
    ...
    pub fn model_with_type(
        ...
        let spec = ModelSpec::with_type(provider, provider_api, model);
        self.main_selection = Some(RoleSelection::new(spec));
        self
    }
