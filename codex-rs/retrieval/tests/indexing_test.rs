...
"#;
    ...
    // With a small chunk size, CodeSplitter should still try to keep functions whole
    let chunker = CodeChunkerService::new(200);
...
"#;
    ...
    let chunker = CodeChunkerService::new(300);
...
fn test_chunk_overlap_prepends_content() {
    ...
    // With overlap
    let chunker_with_overlap = CodeChunkerService::with_overlap(100, 30);
    ...
    if chunks.len() >= 2 {
        ...
        // (overlap prepends ~30 chars from previous chunk)
        assert!(
...
async fn test_change_detector_detects_new_files() {
    ...
    let changes = detector
        .detect_changes("workspace1", None, &files)
        .await
...
async fn test_change_detector_detects_modified_files() {
    ...
    let changes1 = detector
        .detect_changes(workspace, None, &files)
        .await
        .expect("detect_changes failed");
    ...
    for change in &changes1 {
        if let Some(hash) = &change.content_hash {
            detector
                .update_catalog(workspace, None, &change.filepath, hash, 0, 1, 0)
                .await
