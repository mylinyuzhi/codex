...
//!
//! Spawned by `spawn_shell_task` for every background shell. Every
//! [`STALL_CHECK_INTERVAL_MS`] checks whether the on-disk output has
//! grown; if it's been frozen for [`STALL_THRESHOLD_MS`] AND the
//! tail looks like an interactive prompt, fires a stall notification
//! and exits.
//!
//! TS source: `tasks/LocalShellTask/LocalShellTask.tsx:46-104`
//! `startStallWatchdog`. The detection logic
//! ([`matches_interactive_prompt`]) lives in `coco_tasks::stall`.
