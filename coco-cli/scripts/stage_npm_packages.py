...
# `name:` field of the GitHub Actions workflow that publishes coco-<triple>.zst
# artifacts. Used by `gh run list --workflow <NAME>` to locate the release run
...
COCO_PLATFORM_PACKAGES = getattr(_BUILD_MODULE, "COCO_PLATFORM_PACKAGES", {})
...
def tarball_name_for_package(package: str, version: str) -> str:
    if package in COCO_PLATFORM_PACKAGES:
        platform = package.removeprefix("coco-cli-")
        return f"coco-cli-npm-{platform}-{version}.tgz"
...
def main() -> int:
    ...
    try:
        if native_components:
            ...
            vendor_temp_root = Path(tempfile.mkdtemp(prefix="coco-npm-native-", dir=runner_temp))
