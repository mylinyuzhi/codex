...
impl Config {
    ...
    /// Get `ProviderApi` by name.
    pub fn provider_api(&self, name: &str) -> Option<ProviderApi> {
        self.providers.get(name).map(|p| p.api)
    }
    ...
    pub fn all_model_selections(&self) -> Vec<RoleSelection> {
        ...
        for (provider_name, provider_info) in &self.providers {
            for (slug, provider_model) in &provider_info.models {
                ...
                let spec = ModelSpec::with_type(provider_name, provider_info.api, slug)
