//! Live integration tests for cocode-inference.
//!
...
//!
//! Run with: `cargo test -p cocode-inference --test live -- --test-threads=1`
...
async fn test_cross_provider_all() -> Result<()> {
    ...
    for (i, provider_a) in providers.iter().enumerate() {
        for (j, provider_b) in providers.iter().enumerate() {
            ...
            let model_a = match cocode_inference::create_model(
                &provider_a.provider_info,
                &provider_a.model_slug,
            ) {
                Ok(m) => m,
                Err(e) => {
            ...
            let model_b = match cocode_inference::create_model(
                &provider_b.provider_info,
                &provider_b.model_slug,
            ) {
                Ok(m) => m,
                Err(e) => {
            ...
            let client = cocode_inference::ApiClient::new();

            // A generates, B continues with A's response as history
            let request_a = cocode_inference::LanguageModelCallOptions::new(vec![
                cocode_inference::LanguageModelMessage::user_text(
            ...
            let response_a = match client
                .stream_request(
                    &*model_a,
                    request_a,
                    cocode_inference::StreamOptions::streaming(),
                )
                .await
            {
            ...
            let request_b = cocode_inference::LanguageModelCallOptions::new(vec![
                cocode_inference::LanguageModelMessage::user_text(
                    "What is 2+2? Reply with just the number.",
                ),
                cocode_inference::LanguageModelMessage::assistant_text(&text_a),
                cocode_inference::LanguageModelMessage::user_text(
