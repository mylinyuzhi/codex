...
//!
//! TS source: `MessageOrigin` discriminated union (used on `QueuedCommand`
//! and surfaced via `Attachment.origin`) + `wrapCommandText`
//! (`messages.ts:5496-5512`).
//!
...
//! collapsed all of that to one bit — discarding every distinction TS
//! relies on. The typed enum here restores parity end-to-end.
//!
...
/// Origin of a queued command. Mirrors TS `MessageOrigin` exactly,
/// including the `kind`-tagged wire format used by `QueuedCommand` and
/// `Attachment` payloads.
///
/// `None` is treated as [`QueueOrigin::Human`] by [`wrap_command_text`]
/// — TS folds the same way (`messages.ts:5507-5511`).
...
/// TS `wrapCommandText` (`messages.ts:5496-5512`) verbatim — including the
/// em-dash in the channel template.
///
/// `None` is rendered with the human template, matching TS's
/// `case 'human': case undefined: default:` collapse.
