...
fn note_viewport_initializes_without_change() {
    ...
    assert!(!state.replay_needed_for_viewport(80, 10));
}

#[test]
fn note_viewport_reports_changed_after_initial_width() {
    let mut state = HistoryReflowState::default();
    state.note_viewport(80, 10);

    assert_eq!(
        state.note_viewport(100, 10),
        HistoryViewportChange {
            initialized: false,
            changed: true,
        }
    );
    assert!(state.replay_needed_for_viewport(100, 10));
}
...
fn stream_finish_replay_needed_when_resize_was_requested_during_stream() {
    let mut state = HistoryReflowState::default();
    state.schedule_viewport_replay(120, 10, true);
...
fn stream_finish_replay_needed_when_replay_ran_during_stream() {
    let mut state = HistoryReflowState::default();
    state.mark_replayed_viewport(120, 10, true);
...
fn clear_resets_reflow_state() {
    let mut state = HistoryReflowState::default();
    state.note_viewport(80, 10);
    state.schedule_viewport_replay(100, 12, true);
