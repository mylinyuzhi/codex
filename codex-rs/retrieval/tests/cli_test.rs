...
fn test_workspace_name_extraction() {
    ...
    // Normal directory
    assert_eq!(workspace_name(Path::new("/home/user/myproject")), "myproject");
...
async fn test_index_build_empty_directory() {
    ...
    let mut rx = manager
        .rebuild("test", workdir.path(), None, RebuildMode::Incremental)
        .await
        .unwrap();
...
async fn test_clean_rebuild_mode() {
    ...
    // Clean rebuild
    let mut rx = manager
        .rebuild("test", workdir.path(), None, RebuildMode::Clean)
        .await
        .unwrap();
    while let Some(_) = rx.recv().await {}
...
"#,
    ...
    let mut rx = manager
        .rebuild(workspace, workdir.path(), None, RebuildMode::Incremental)
        .await
        .unwrap();
    while let Some(_) = rx.recv().await {}
