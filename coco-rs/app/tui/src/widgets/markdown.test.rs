...
fn test_plain_text() {
    ...
    let lines = markdown_to_lines("Hello world", styles, 80);
...
fn test_code_block_syntax_highlighting_can_be_disabled() {
    ...
    let highlighted = markdown_to_lines_with_syntax(text, styles, 80, SyntaxHighlighting::Enabled);
    assert!(
        highlighted[1]
            .spans
            .iter()
    ...
    let plain = markdown_to_lines_with_syntax(text, styles, 80, SyntaxHighlighting::Disabled);
    assert!(
        !plain[1]
            .spans
            .iter()
