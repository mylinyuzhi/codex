...
A simple TUI for testing `coco-lsp` functionality and inspecting internal LSP operations.

## Quick Start

```bash
# Build with TUI feature
cargo build -p coco-lsp --features tui

# Run with a workspace directory
cargo run -p coco-lsp --features tui -- /path/to/rust/project

# Or specify an initial file
cargo run -p coco-lsp --features tui -- /path/to/project -f src/lib.rs
```
...
1. Start TUI: `cargo run -p coco-lsp --features tui -- .`
...
2. Check logs (output to stderr):
   ```bash
   cargo run -p coco-lsp --features tui -- . 2>lsp.log
   ```
...
The TUI is implemented as an optional feature of the `coco-lsp` crate:
...
cargo check -p coco-lsp --features tui
```
