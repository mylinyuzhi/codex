        ...
        match item {
            ...
            ResponseItem::FunctionCall {
                ...
                if let Some(sig_base64) = collected_call_signatures.get(call_id) {
                    if let Ok(sig_bytes) = base64::engine::general_purpose::STANDARD.decode(sig_base64) {
                ...
                if let Some(encrypted) = encrypted_content {
                    if let Ok(sig_data) = serde_json::from_str::<serde_json::Value>(encrypted) {
                        ...
                        if let Some(msg_sig_base64) = sig_data
                            ...
                            .filter(|s| !s.is_empty())
                        {
                            if let Ok(sig_bytes) = base64::engine::general_purpose::STANDARD.decode(msg_sig_base64) {
                        ...
                        current_parts.push(Part::with_thought_signature(encrypted.as_bytes().to_vec()));
                    }
                }
...
pub fn response_to_events(response: &GenerateContentResponse) -> (Vec<ResponseEvent>, String) {
    ...
    if !text_parts.is_empty() {
        ...
        events.push(ResponseEvent::OutputItemDone(ResponseItem::Message {
            ...
            content: vec![ContentItem::OutputText { text: combined_text }],
        }));
    }
...
fn generate_call_id() -> String {
    format!("{}{}", CLIENT_GENERATED_CALL_ID_PREFIX, uuid::Uuid::new_v4())
}
...
mod tests {
    ...
    fn test_round_trip_multiple_function_calls_with_signatures() {
        ...
        let model_content = contents.iter().find(|c| c.role == Some("model".to_string()));
    ...
    fn test_function_call_missing_id_gets_client_generated() {
        ...
        let fc_event = events.iter().find(|e| {
            matches!(e, crate::common::ResponseEvent::OutputItemDone(
                ResponseItem::FunctionCall { .. }
            ))
        });
        ...
        let user_content = contents
            ...
            .find(|c| {
                c.role == Some("user".to_string())
                    && c.parts
                        .as_ref()
                        .map_or(false, |p| p.iter().any(|part| part.function_response.is_some()))
            })
    ...
    fn test_reasoning_roundtrip_with_call_signatures() {
        ...
        let reasoning = items.iter().find(|item| {
            matches!(item, ResponseItem::Reasoning { .. })
        });
        assert!(reasoning.is_some(), "Should have Reasoning item");

        if let Some(ResponseItem::Reasoning { encrypted_content, .. }) = reasoning {
        ...
        let model_content = contents.iter().find(|c| c.role == Some("model".to_string()));
        assert!(model_content.is_some(), "Should have model content in roundtrip");
    ...
    fn test_binary_signature_roundtrip() {
        ...
        let reasoning = items.iter().find(|item| matches!(item, ResponseItem::Reasoning { .. }));
        assert!(reasoning.is_some(), "Should have Reasoning item");

        let encrypted_content = if let ResponseItem::Reasoning { encrypted_content, .. } = reasoning.unwrap() {
        ...
        let sig_data: serde_json::Value = serde_json::from_str(&enc).expect("valid JSON");
        let msg_sig_base64 = sig_data.get("message_signature").and_then(|v| v.as_str()).unwrap();
        let call_sigs = sig_data.get("call_signatures").and_then(|v| v.as_object()).unwrap();
        let call_sig_base64 = call_sigs.get("call_binary").and_then(|v| v.as_str()).unwrap();

        // Verify base64 can be decoded back to original bytes
        use base64::Engine;
        let decoded_msg_sig = base64::engine::general_purpose::STANDARD.decode(msg_sig_base64).expect("decode message sig");
        let decoded_call_sig = base64::engine::general_purpose::STANDARD.decode(call_sig_base64).expect("decode call sig");

        assert_eq!(decoded_msg_sig, binary_sig, "Message signature should roundtrip correctly");
        assert_eq!(decoded_call_sig, binary_sig, "Call signature should roundtrip correctly");
