...
//! verbatim, which means there's no Layer-1 string repair here.
...
//!    JSON inside `input` (an occasional cross-protocol leak), Layer 1
//!    still passes the string through. The recovery is Layer 2's
//!    job (`normalize_value_string` in `app/query`). This test pins
//!    the Layer 1 contract: the string is **not silently mangled**
...
async fn anthropic_nonstream_value_string_passes_through_unmangled() {
    ...
    // arrives at the adapter as a `Value::String`. Layer 1 does NOT
    // recursively parse — `normalize_value_string` at Layer 2
    // (`app/query::tool_input_validate`) handles that case. Test
    // pins the Layer 1 contract: the raw string survives intact.
...
async fn anthropic_nonstream_value_string_with_markdown_fence_passes_through() {
    ...
    // JSON inside `input`. Layer 1 doesn't unwrap; Layer 2 does.
...
async fn anthropic_nonstream_null_input_preserved() {
    ...
    // occasionally emits `null` on degenerate completions. Layer 1
    // passes through — Layer 2 schema validation catches the type
