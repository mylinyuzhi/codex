...
fn test_session_new() {
    ...
    assert_eq!(session.provider_api(), Some(ProviderApi::Openai));
...
fn test_session_with_id() {
    ...
    assert_eq!(session.provider_api(), Some(ProviderApi::Anthropic));
}
...
fn test_session_builder_with_type() {
    let session = Session::builder()
        .working_dir("/project")
        .model_with_type("my-custom-openai", ProviderApi::Openai, "gpt-5")
        .build();
...
fn test_session_serde() {
    ...
    assert_eq!(parsed.provider_api(), session.provider_api());
}
