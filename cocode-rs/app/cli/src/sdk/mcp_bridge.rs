...
//!
//! Routes tool calls for SDK-managed MCP servers through the NDJSON
//! control channel instead of direct MCP connections. This enables
...
//! 4. Turn loop picks it up, writes `ServerRequest::McpRouteMessage` to stdout
//! 5. SDK client handles the call, responds with `McpRouteMessageResponse`
//! 6. Turn loop calls `bridge.resolve()` to unblock the caller
...
/// Bridge between SDK MCP tool wrappers and the NDJSON control channel.
...
/// Tool wrapper for SDK-managed MCP tools routed through the control channel.
...
impl Tool for SdkMcpToolWrapper {
    ...
    async fn execute(&self, input: Value, _ctx: &mut ToolContext) -> Result<ToolOutput> {
        tracing::debug!(
            ...
            tool = %self.tool_name,
            "Executing SDK MCP tool via control channel"
        );
