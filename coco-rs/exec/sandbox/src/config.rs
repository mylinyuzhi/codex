...
///
/// TODO: Move to `coco-types` once that crate is implemented.
/// This is the user's intent; [`EnforcementLevel`] is the runtime behavior.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum SandboxMode {
    /// Read-only: file writes are blocked.
    ReadOnly,
    /// Workspace-write: writes allowed to project directories.
    WorkspaceWrite,
    /// Full access: no restrictions (sandbox disabled).
    FullAccess,
    /// External sandbox: enforcement delegated to an external tool (e.g., Docker).
    ExternalSandbox,
}
