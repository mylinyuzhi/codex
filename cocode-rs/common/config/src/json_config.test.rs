...
fn test_app_config_parse_minimal() {
    ...
    assert_eq!(models.get(ModelRole::Main), Some("openai/gpt-5"));
}
...
fn test_app_config_parse_full() {
    ...
    let config: AppConfig = serde_json::from_str(json_str).unwrap();
    let models = config.models.as_ref().unwrap();
    assert_eq!(models.get(ModelRole::Main), Some("genai/gemini-3-pro"));
...
fn test_app_config_parse_with_profiles() {
    ...
    assert_eq!(
        anthropic_models.get(ModelRole::Main),
    ...
    assert_eq!(fast_models.get(ModelRole::Main), Some("openai/gpt-5-mini"));
...
fn test_selected_profile() {
    ...
    assert_eq!(models.get(ModelRole::Main), Some("openai/gpt-5-mini"));
}
...
fn test_resolve_with_models_field() {
    ...
    let fast = resolved.models.get_direct(ModelRole::Fast).unwrap();
    assert_eq!(fast.provider, "anthropic");
    assert_eq!(fast.slug, "claude-haiku");

    let vision = resolved.models.get_direct(ModelRole::Vision).unwrap();
...
fn test_app_config_parse_with_models() {
    ...
    let models = config.models.as_ref().unwrap();
    assert!(models.get(ModelRole::Main).is_some());
    assert!(models.get(ModelRole::Fast).is_some());
}
