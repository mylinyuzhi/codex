...
async fn run_repl(
    ...
    if let Some(path) = config_path {
        ...
        println!("Config: {}/.codex/retrieval.toml (or ~/.codex/retrieval.toml)", workdir.display());
    }
...
async fn cmd_build(workdir: &PathBuf, config: &RetrievalConfig, clean: bool) -> anyhow::Result<()> {
    ...
    let mut rx = manager.rebuild(workspace, workdir, None, mode).await?;
    ...
    while running.load(Ordering::SeqCst) {
        if let Some(events) = watcher.recv_timeout(Duration::from_millis(500)) {
            ...
            if count > 0 {
                ...
                let mut rx = manager
                    .rebuild(workspace, workdir, None, RebuildMode::Incremental)
                    .await?;
...
fn cmd_config(config: &RetrievalConfig) -> anyhow::Result<()> {
    ...
    println!();
    println!("Chunking:");
    println!("  max_chunk_size: {}", config.chunking.max_chunk_size);
    println!("  chunk_overlap: {}", config.chunking.chunk_overlap);
    println!(
        "  smart_collapse: {}",
        config.chunking.enable_smart_collapse
    );
    println!();
    println!("Search:");
