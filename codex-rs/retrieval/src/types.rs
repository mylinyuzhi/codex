...
impl CodeChunk {
    /// Prepare content for embedding with filepath context.
    ///
    /// Wraps the chunk content with filepath information so that embeddings
    /// can understand the file context. This is inspired by Tabby's approach.
    ///
    /// Format: ```{filepath}\n{content}\n```
    ///
...
/// Index tag for workspace and branch tracking.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct IndexTag {
    /// Workspace identifier
    pub workspace: String,
...
mod tests {
    ...
    fn test_code_chunk_with_metadata() {
        ...
        assert_eq!(chunk.branch, Some("main".to_string()));
