...
| `state/`, `bootstrap/`, `components/`, `screens/`, `ink/`, `outputStyles/`, `entrypoints/`, `cli/`, `server/`, `bridge/` | `coco-state`, `coco-session`, `coco-tui`, `coco-cli`, `coco-bridge` | `crate-coco-app.md` | v1 |
...
**Copy from cocode-rs** (39 crates, no plan doc needed): `coco-error`, 24 `utils/*`, 8 `vercel-ai/*`, `coco-sandbox`, `coco-process-hardening`, `coco-lsp`, `coco-retrieval`
...
| P1 | coco-otel L3: ~53 应用事件 (query/session/config/oauth/mcp 生命周期) | Phase 3 |
