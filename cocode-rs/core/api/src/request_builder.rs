...
impl RequestBuilder {
    ...
    pub fn build(mut self) -> LanguageModelCallOptions {
        let provider_type = self.context.model_spec.provider_type;

        // Step 1: Per-provider message normalization (empty content, tool ID sanitization)
        message_normalize::normalize_prompt(&mut self.prompt, provider_type);
        ...
        opts.temperature = self
            ...
            .or_else(|| self.context.temperature().map(|t| t as f32));
        ...
        opts.top_p = self
            .top_p_override
            .or_else(|| self.context.top_p().map(|p| p as f32));
        ...
        if let Some(ref mut tools) = self.tools {
            schema_sanitize::sanitize_tool_schemas(tools, provider_type);
        }
        ...
        let mut provider_options = request_options_merge::provider_base_options(provider_type);
