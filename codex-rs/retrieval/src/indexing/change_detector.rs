...
impl ChangeDetector {
    ...
    pub async fn detect_changes(
        ...
        // Get catalog entries
        let catalog = self.get_catalog(workspace, branch).await?;
    ...
    pub async fn get_catalog(
        ...
        self.db
            .query(move |conn| {
                let (sql, params): (&str, Vec<String>) = if let Some(ref b) = br {
                    (
                        "SELECT filepath, content_hash, mtime, indexed_at, chunks_count, chunks_failed
                         FROM catalog
                         WHERE workspace = ? AND branch = ?",
                        vec![ws.clone(), b.clone()],
                    )
                } else {
                    (
                        "SELECT filepath, content_hash, mtime, indexed_at, chunks_count, chunks_failed
                         FROM catalog
                         WHERE workspace = ? AND branch IS NULL",
                        vec![ws.clone()],
                    )
                };
    ...
    pub async fn update_catalog(
        ...
        self.db
            .query(move |conn| {
                conn.execute(
                    "INSERT OR REPLACE INTO catalog (workspace, branch, filepath, content_hash, mtime, indexed_at, chunks_count, chunks_failed)
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                    rusqlite::params![ws, br, fp, hash, mtime, now, chunks_count, chunks_failed],
                )?;
                Ok(())
            })
            .await
    ...
    pub async fn remove_from_catalog(
        ...
        self.db
            .query(move |conn| {
                if br.is_some() {
                    conn.execute(
                        "DELETE FROM catalog WHERE workspace = ? AND branch = ? AND filepath = ?",
                        rusqlite::params![ws, br, fp],
                    )?;
                } else {
                    conn.execute(
                        "DELETE FROM catalog WHERE workspace = ? AND branch IS NULL AND filepath = ?",
    ...
    pub async fn needs_reindex(
        ...
        self.db
            .query(move |conn| {
                let result: Option<i64> = if br.is_some() {
                    conn.query_row(
                        "SELECT mtime FROM catalog WHERE workspace = ? AND branch = ? AND filepath = ?",
                        rusqlite::params![ws, br, fp],
                        |row| row.get(0),
                    )
                    .ok()
                    ...
                    conn.query_row(
                        "SELECT mtime FROM catalog WHERE workspace = ? AND branch IS NULL AND filepath = ?",
                        rusqlite::params![ws, fp],
                        |row| row.get(0),
                    )
                    .ok()
...
mod tests {
    ...
    async fn test_detect_changes() {
        ...
        let changes = detector.detect_changes("ws", None, &current).await.unwrap();
        assert_eq!(changes.len(), 2);
        assert!(changes.iter().all(|c| c.status == ChangeStatus::Added));

        // Add to catalog
        detector
            .update_catalog("ws", None, "file1.rs", "hash1", 1000, 5, 0)
            .await
            .unwrap();
        detector
            .update_catalog("ws", None, "file2.rs", "hash2", 1000, 3, 0)
            .await
            .unwrap();
        ...
        let changes = detector
            .detect_changes("ws", None, &current_modified)
            .await
            .unwrap();
        ...
        let changes = detector
            .detect_changes("ws", None, &current_deleted)
            .await
            .unwrap();
