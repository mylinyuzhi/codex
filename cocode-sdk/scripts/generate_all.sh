...
#
# Usage:
#   ./cocode-sdk/scripts/generate_all.sh          # Generate all
#   ./cocode-sdk/scripts/generate_all.sh --check   # Verify generated files are up-to-date
...
if $CHECK_MODE; then
    ...
    PYTHON_OUTPUT="$REPO_ROOT/cocode-sdk/python/src/cocode_sdk/generated/protocol.py"
    TEMP_DIR="$(mktemp -d)"
    ...
    if ! diff -q "$TEMP_DIR/protocol.py.before" "$PYTHON_OUTPUT" > /dev/null 2>&1; then
        ...
        echo "Run: ./cocode-sdk/scripts/generate_all.sh"
